...
# Load the lowload CSVs
csv_files = sorted(glob.glob("gpu_metrics_mobilenet_low_run*.csv"))
...
plt.xticks(x, labels)
plt.title("MobileNet Low Load - Power Variation Across 3 Runs")
plt.ylabel("Power (Watts)")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig("mobilenet_lowload_variation.png")
plt.show()
